...
export default function AdminExpense() {
  ...
  useEffect(() => {
    axios.get("http://localhost:8989/admin/expenses", {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8989/admin/expense/${id}`, {
        headers: { Authorization: "Bearer " + token }
      });
  ...
  return (
    <div>
      <h2 style={title}>💸 Expense Records</h2>
          ...
          <tbody>
            {data.map((e, idx) => (
              <tr key={idx} style={row}>
                ...
                <td>
                  <button style={viewBtn} onClick={() => handleView(e)}>
                    View
                  </button>

                  <button style={deleteBtn} onClick={() => handleDelete(e.id)}>
                    Delete
                  </button>
                </td>
              </tr>
